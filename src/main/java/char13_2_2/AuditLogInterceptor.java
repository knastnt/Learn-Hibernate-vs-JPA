...
public class AuditLogInterceptor extends EmptyInterceptor {
    ...
    //Вызывается когда экземпляр становится хранимым
    ...
    //Вызывается при изменении состояния сущности
    public boolean onFlushDirty(Object entity, Serializable id,
        ...
        return false; //Мы не изменяли текущего состояния
    }
    ...
    public void postFlush(Iterator iterator) throws CallbackException {
        ...
        try{
            for (Auditable entity : inserts) {
                tempSession.persist(
                        new AuditLogRecord("insert", entity.getId(), currentUserId)
                );
            }
            for (Auditable entity : updates) {
                tempSession.persist(
                        new AuditLogRecord("update", entity.getId(), currentUserId)
                );
            }
            tempSession.flush();
        } finally {
